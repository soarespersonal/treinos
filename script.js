...
async function buscarTreinosParaEdicao() {
    ...
    const { data: treinos } = await supabaseClient
        ...
        .select('*')
        .eq('aluno_id', alunoId)
        .order('letra_treino', { ascending: true })
    ...
    treinos?.forEach(t => {
        container.innerHTML += `
            <div style="background: #252525; padding: 15px; border-radius: 8px; border-left: 4px solid #ffcc00; margin-bottom:10px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 5px; margin: 10px 0;">
                    <input type="number" value="${t.series}" id="edit-ser-${t.id}" title="Séries">
                    <input type="text" value="${t.reps}" id="edit-rep-${t.id}" title="Reps">
                    <input type="number" value="${t.descanso}" id="edit-des-${t.id}" title="Descanso">
                    <input type="number" value="${t.ordem || 0}" id="edit-ordem-${t.id}" title="Ordem">
                </div>
...
async function salvarEdicao(id) {
    const { error } = await supabaseClient.from('treinos').update({
        ...
        ordem: parseInt(document.getElementById(`edit-ordem-${id}`).value)
    }).eq('id', id);

    if (error) alert("Erro!"); 
    else {
        alert("Atualizado!");
        buscarTreinosParaEdicao();
    }
}
...
async function clonarTreinoCompleto() {
    ...
    if (origemId === destinoId) return alert("Origem e destino iguais.");

    if (confirm("Clonar todos os exercícios?")) {
        const { data: treinosOrigem } = await supabaseClient.from('treinos').select('*').eq('aluno_id', origemId);

        if (!treinosOrigem || treinosOrigem.length === 0) return alert("Nada para copiar.");

        const novosTreinos = treinosOrigem.map(t => {
            const { id, ...dadosSemId } = t;
            return { ...dadosSemId, aluno_id: destinoId, carga: "" };
        });

        const { error } = await supabaseClient.from('treinos').insert(novosTreinos);
        if (error) alert(error.message); else alert("Treino Clonado!");
    }
}
